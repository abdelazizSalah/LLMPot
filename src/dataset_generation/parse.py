...
def parse_with_file(
    ...
    # --- NEW: sequential split into train/val/test ---
    # Train: [0, max_iter)
    __parse(protocol, port, cap, f"{csv}_train", context_length, has_time, experiment,
            start_index=0, max_iter=max_iter)

    # Val: [max_iter, 2*max_iter)
    __parse(protocol, port, cap, f"{csv}_val", context_length, has_time, experiment,
            start_index=max_iter, max_iter=max_iter)

    # Test: [2*max_iter, 3*max_iter)
    __parse(protocol, port, cap, f"{csv}_test", context_length, has_time, experiment,
            start_index=2 * max_iter, max_iter=max_iter)

    # Important: close capture (pyshark spawns tshark)
    try:
        cap.close()
    except Exception:
        pass
...
def __parse(
    ...
    offset = 0.0
    prev_time = None  # --- change: None means “not initialized yet” ---

    end_index = start_index + max_iter
    print(f"Window: start_index={start_index}, end_index(exclusive)={end_index}")

    print('Iterating over packets...')
    for index, packet in tqdm(enumerate(cap)):
        # Skip until we reach the start of this split
        if index < start_index:
            continue

        # Stop when the split window ends
        if index >= end_index:
            print(f"Reached end of window at index {index}.")
            break
        ...
        # --- IMPORTANT: reset time offset per split ---
        # Each split (train/val/test) will have its own time starting at 0.
    ...
    if context_length > 0:
        ...
        if has_time:
            with open(f"{DATASET_PARSED_Custom}/{experiment}/{csv_filename}.csv", "w") as csv_context:
                ...
                for i in range(0, len(dataset_df)):
                    csv_context.write(
                        f"{'%.2f' % (dataset_df[SOURCE_TEXT][i]['offset'])}|{dataset_df[SOURCE_TEXT][i]['packet']}:,"
                        f"{dataset_df[TARGET_TEXT][i]}\n"
                    )
        else:
            dataset_df.to_csv(f"{DATASET_PARSED_Custom}/{experiment}/{csv_filename}.csv", index=False)
